...
RUN_ID="$(gh run list --repo silverwolfdoc/herdr-usage-bar --workflow CI --branch main --commit "$HEAD_SHA" --limit 1 --json databaseId --jq '.[0].databaseId')"
...
gh run watch "$RUN_ID" --repo silverwolfdoc/herdr-usage-bar --exit-status
